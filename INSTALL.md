...
- `plotrail`：从已批准的 canon 和章节合同出发，规划、起草、审稿并维护长篇小说。
- `plotrail-intake`：诊断已有正文、摄取旧稿、精修草稿、学习作者风格、审查连续性，并为 PlotRail 准备精简交接。
...
cp -R plotrail/plotrail ~/.claude/skills/plotrail
...
Copy-Item -Recurse -Force ".\plotrail\plotrail" "$env:USERPROFILE\.codex\skills\plotrail"
...
```bash
python3 ~/.codex/skills/plotrail/scripts/init_novel_project.py /tmp/plotrail-demo
```
...
1. 需要规划和写新章节时，打开 `plotrail/SKILL.md`。
2. 需要诊断、摄取、精修和交接已有稿件时，打开 `plotrail-intake/SKILL.md`。
3. 只加载当前任务需要的 `references/` 文件，避免把所有说明都塞进上下文。
4. 从零开始长篇项目时，可用 `plotrail/scripts/init_novel_project.py` 创建稳定的项目目录。
